...
**Version:** Draft 1.3 (living-site desks + public-surface refresh)  
**Date:** September 2026  
**Prior:** Draft 1.2 (2026-09-10); Draft 1.1 (2026-09-09); Draft 1.0 (2026-09-09)  

---

## Abstract

This practice-based doctoral dissertation synthesizes knowledge produced by owning computing pipelines end-to-end: from metal-adjacent graphics and input systems, through shipped games and tools, to platform economics and living networked culture. The primary evidence base is a verified public inventory of systems built and maintained by Steven Philley under Color It Company LLC / Flux Studios—most notably the Ra Engine (Vulkan 1.4 hardware ray-tracing editor and game engine; Steam app 4732830), Digiflux titles Launch Up and Laser Tag, the competitive tetromino title Ablockalypse (custom SDL engine), a worldwide active BBS directory, and adjacent media systems: photography Vol. I plus the mid-September 2026 living site on stevenphilley.com (local literacy, weather instruments, science/astronomy desks, Annual Dial, suggestion board and product surface, maths/tools/games) and cultural learning sites.
...
Methodologically, the work follows practice-based research traditions (Schön’s reflective practice; Candy and Edmonds on practitioner research; extended in Drafts 1.1–1.2 via Candy 2019, Smith & Dean, Borgdorff, Nelson, Driscoll, Swink close readings, and related networked-culture works) while treating public artifacts, documentation, storefront statements, repository headers, dated JSON snapshots, and live HTTP fetches of public URLs as primary evidence. Private biography that cannot be verified from public sources is marked with *Practitioner note / open for author expansion* callouts rather than invented. Technical chapters stay close to digiflux.one documentation, Steam Early Access disclosures, Color It Company public pages, and related URLs.
...
1. This document is a **self-directed practice-based doctoral dissertation**. It uses doctoral form as a vehicle for synthesizing and testing practitioner knowledge. It is **not** a claim of an awarded institutional PhD. Draft 1.3 does not change this framing.
...
11. [Chapter 8 — Media adjacency](#chapter-8-media-adjacency-photography-and-cultural-systems-as-attention-craft) *(Draft 1.3: living-site desks)*  
...
| T18 | Living stevenphilley.com desks (Draft 1.3 live-fetch) |
| T19 | BBS Snapshot C (2026-09-12) vs A/B |
...
6. **An adjacency argument** that photography, cultural learning sites, and (from Draft 1.3) living local/science/weather/dial/suggest desks train the same attention and systems craft as lighting and feel (Chapter 8; Grade E where interpretive).
...
In scope: public Digiflux/Ra documentation; Steam disclosures; CIC/itch pages (including the 2026-09-15 `colorit.co` cutover); BBS directory page and dated JSON snapshots; photography and the living stevenphilley.com desks verified live on main; panatau public site; LinkedIn-public career framing; real secondary literature. Pending GitHub PRs are cited as pending, not as published pages.
...
- **Grade E — Interpretive adjacency:** photography, cultural sites, and living local/science/weather/dial desks as attention/systems craft (clearly marked interpretive).
...
## 2.12 Draft 1.3 source discipline

Draft 1.3 adds a living-site layer without relaxing Draft 1.2 rules:
...
## 3.10b Placeful instruments after the feed (Draft 1.3, Grade E)

Chapter 3’s opening claim is that pre-feed networked culture was often *placeful*. Draft 1.3 records a contemporary rhyme on the same maker’s public site: Campbell-default weather, Pruneyard land-history literacy, liquefaction and earthquake desks, and a Bay Area local-sky chart. These are **not** BBS software and not engine subsystems. The adjacency is epistemic: a practice that already refuses invented boards and silent UBO lies now also refuses unsourced neighborhood myth and unsourced sky claims. Interpretive only—Grade E—until the author maps a named session (orchard dusk, Pacific rim) onto a named Ra control.
...
### Snapshot C (Draft 1.3 window, fetched 2026-09-15)

Live fetch of `https://coloritcompany.com/bbs/bbslist.json` followed a redirect to `https://colorit.co/bbs/bbslist.json` (HTTP 200). Official top-level fields:[^bbs-json][^colorit-co]

| Field | Value |
| --- | --- |
| `generated` | **2026-09-12** |
| `probedAt` | 2026-09-11T18:43:17Z |
| `count` | **1323** |
| `statusCounts` | green **1091**, red **207**, yellow **22** |
| `sources` | still **17** |
| Top languages (system tally) | English 841; Unknown 161; German 62; Chinese 51; Spanish 35; … |
| Top software (system tally) | Synchronet **419**; Mystic **230**; Custom 61; Worldgroup 45; Enigma 1/2 41; … |

Two honesty notes, not errata to hide:

1. **Domain cutover.** The historical handle `coloritcompany.com/bbs` still works as a URL; the storefront of record for the HTML directory and JSON on this fetch is `colorit.co`. Cite both; do not pretend the old hostname vanished.
2. **Yellow appears.** Snapshot B recorded yellow **0**. Snapshot C’s `statusCounts` include **22** yellow. That is probe-state weather, not a new product claim. Per-system `status` tallies on this file were green 1093 / red 207 / yellow 22 / unset 1 versus `statusCounts.green` 1091—another small snapshot hygiene drift. Prefer the official `statusCounts` + `count` fields; do not average.

A→B→C in one week (1304 → 1322 → 1323; green 1092 → 1099 → 1091) is the present-tense archive continuing. Treat 1323 as dated, not eternal.
...
4. **Present-tense archive** — Snapshot A 1304/1092 (2026-09-09) → Snapshot B 1322/1099 (2026-09-10) → Snapshot C 1323 / 1091 green (generated 2026-09-12; fetched 2026-09-15).
...
- Treating 1322 or 1323 as eternal.
...
A narrow software monograph might ignore stevenphilley.com and panatau.com. This practice-based dissertation includes them because attention is the shared substrate of lighting, feel, and cultural explanation. The same maker who calibrates inverse-square intensities also sequences California light from warm to cool, builds cultural learning structure for Romanian history, crafts, dialect, and maps, and—by Draft 1.3’s live-fetch window—publishes local-literacy desks, weather and sky instruments, and a suggestion board on the same domain.[^photo][^panatau][^inventory][^sp-local][^sp-weather]
...
**Draft 1.3 hostname note (2026-09-15):** fetching `https://coloritcompany.com/` now **redirects to `https://colorit.co/`**. The live hub title remains “Color It Company — Independent Media & Games.” Suggestion-page prices ($39/yr, $39/mo) appear on that hub HTML. Historical Draft 1.2 citations of the coloritcompany.com hostname still resolve via redirect; cite the cutover rather than pretending the old host vanished.[^colorit-co]

Notably, the hub scan does **not** prominently list Ra Engine / Launch Up / Digiflux—those live on digiflux.one + Steam. That split is **studio multi-surface identity**: CIC / colorit.co as media/games face; Digiflux as engine/platform face; same LLC practice underneath. Owned Pipelines readers should not demand a single homepage that contains every artifact; they should demand *traceable* surfaces (which this inventory supplies).
...
**Claim C8:** Photography and cultural learning sites are not digressions from engine research; they train and express the same disciplined attention that theatrical RT lighting and competitive feel require. Media adjacency is part of the epistemology of ownership. Draft 1.3 extends the claim to living local/science/weather/dial/suggest desks on stevenphilley.com—still Grade E where the parallel is interpretive, Grade A where URLs and disclaimers are simply fetched.
...
## 8.10 Draft 1.3: the living site beyond photography Vol. I

Draft 1.2 read stevenphilley.com chiefly as **Vol. I — California Light** (nine images, warm→cool). That reading remains. What densified by mid-September 2026 is that the same domain is now a **desk city**: Local literacy, Weather, Science/astronomy, Annual Dial, Suggest/suggestions, maths, tools, games, electronics, film studies, health, family, public photos, news, and a `/whats-new/` changelog—verified live on `main` by HTTP 200 fetches on 2026-09-15 UTC, not by inventing a sitemap that the live host does not serve.[^whats-new][^sp-local]

**Grade E (interpretive, required):** treating weather ZIP share or a Pruneyard arsenic-literacy essay as “the same as” Vulkan BLAS ownership would be category error. The honest adjacency is narrower: these desks are **client-side instruments with public data contracts and bounded disclaimers**. They rhyme with Ra pitfall tables and BBS “connection ≠ login” copy because they refuse silent overclaim. They do not prove engine skill.

The site’s own `/whats-new/` log (33 fallback entries on the 2026-09-15 fetch) is a public reflective diary of what landed, newest first—Candy-adjacent documentation of making, grepable in HTML.[^whats-new][^candy-2019]

## 8.11 Local desk literacy (Campbell / South Bay)

Live on main (HTTP 200, 2026-09-15):[^sp-local]

| Path | What the public page is | Bound |
| --- | --- | --- |
| `/local/` | Campbell-area hub: traffic, safety, live weather, air quality, groceries, neighborhood notes; doors into Pruneyard, earthquakes, liquefaction | Place desk, not a news wire of record |
| `/local/pruneyard/` | Brynteson prune-and-pear orchards → open-air center; Union Avenue 1982–1985 housing; historic **lead-arsenate** literacy; Santa Clara Valley wetland / marsh **fill** | Cited educational essay. **Not** a parcel diagnosis, soil report, or medical/legal/engineering advice |
| `/local/earthquakes/` | Bay Area quake timeline with Campbell / South Bay focus; USGS-linked | Not a live alert feed |
| `/local/liquefaction/` | USGS-cited plain-language essay on soft ground in a quake | Not a site assessment |
| `/local/groceries/` | Late-summer / early-fall produce notes for Campbell and the South Bay | Seasonal notes, not a price index |
| `/local/nuts/` | Small local desk (civic/lore notes; later grocery cross-link) | Not a crop census |
...
`/weather/` is a live US weather desk: city name or 5-digit ZIP in one box, Open-Meteo in the browser (no API key), current conditions plus a 10-day high/low graph, default **Campbell / 95008**.[^sp-weather] Boot reads `?q=`, `?place=`, or `?zip=` from the URL. After the 2026-09-15 merge of weather ZIP-in-Share (PR #9), the page decorates share URLs with `?zip=` when a five-digit ZIP is known, else `?q=`, and `history.replaceState` keeps the address bar honest.[^sp-weather] Live fetch of `https://stevenphilley.com/weather/?zip=95008` returned HTTP 200.

A Local-desk weather card and AQI card (US AQI, PM2.5, ozone via Open-Meteo; AirNow / KQED map links) sit on `/local/` as companion instruments.[^sp-local]

**Grade E:** a weather instrument that publishes its query contract and refuses a rented key is adjacent to CameraUBO / `bbslist.json` honesty—the handshake is public. It is not a claim that Open-Meteo is an owned renderer.

## 8.13 Science and astronomy desks

Verified HTTP 200 on 2026-09-15:[^sp-astro]

| Path | Public claim (paraphrase) |
| --- | --- |
| `/science/` | Status hub (nanomaterials, robotics, space) plus doors into astronomy and Earth systems; live headlines pointed at `/news/science.html` |
| `/science/astronomy/` | Clickable solar-system schematic (Sun–Neptune; Pluto labeled dwarf). Compressed scale; **not** a precision ephemeris |
| `/science/astronomy/moon/` | Synodic-month phase dial; illumination cartoon; Campbell sky-brightness notes |
| `/science/astronomy/local-sky/` | Approximate orientation for Campbell ≈37.29°N, 121.95°W; month + local-time controls; client-side sidereal math |
| `/science/astronomy/nearest-stars/` | Neighborhood stars with distances / spectral types; some marked invisible from Campbell |
| `/science/astronomy/exoplanets/` | Discovery-method cartoons + schematic explorer of NASA-public confirmed examples |
| `/science/astronomy/galaxy/` | Milky Way starter; Sol in the Orion Arm |
| `/science/atmosphere/` | Clickable cross-section of Earth’s air |

Also live on the science cluster (sitemap + fetches): light-pollution literacy, mining / lithium lesson, ocean-metals, waves.[^whats-new]

Educational bounds are printed on the pages (schematic, not catalog dump, not medical, not engineering). That is the same refusal-of-cosplay tone as BBS probe copy.

## 8.14 Annual Dial (`/circular-calendar.html`)

The Annual Dial is a calendar built like a clock: whole year as one engraved plate, or a **month dial** with live hands; click a day to mark it.[^sp-dial] Theme skins on the live page: **brass, LED, vapor, Columbus, Halloween, Christmas, neon**. Theme id persists under `annual-dial-theme`. Share URLs carry the dial skin (`?theme=`).

Draft 1.3 window (already on `main` when this file was drafted):

- **Columbus** skin (PR #8, merged 2026-09-15) — public theme button, not a historical monograph.
- **Month-dial UX** (PR #10, merged 2026-09-15) — entering month view auto-selects **today** (same as the Today button) so the elapsed band is visible without an extra tap; prev/next in month view still browse other months.

**Grade E:** a year-as-clock is attention choreography (orientation under a chosen atmospheric contract), adjacent to ACES/Gamma and panatau theme modes—not a shipping metric.

## 8.15 Suggest board and commercial suggestion-page surface
...
## 8.16 Maths, tools, games (inventory without play metrics)

Verified live; no fabricated session counts:[^sp-maths][^sp-games]

- **Maths** `/maths/` — interactive trig (unit circle), linear algebra (matrices/vectors), calculus (derivatives, integrals, limits). Browser study desk, not a textbook replacement.
- **Tools** `/tools/` — everyday converters hub; `/tools/temperature.html` is Fahrenheit ↔ Celsius with presets. Also on the older tools spine: Annual Dial, MarketPulse, percentages, Pomodoro, Stilltype typing tutor, Nine Chakras (sitemap).
- **Games** `/games/` — public cards for Checkers, Chess, Tetris, FPS, Maze, Fantasy Hockey, and Colonize (local play, no accounts). Hub lede still says “Six browser games” while listing seven playables and “Six games · Seven addresses”—small **COPY DRIFT**; cite both; invent no DAU.

**Electronics** `/electronics/` is a growing interactive bench (Ohm/power, circuit explorer, resistor/LED codes, Z80 hub, rectifier/zener/peak-detector, and related lessons). **Film studies** `/filmstudies/` is an academic desk (form, history, television, genres, theory, watch, glossary), distinct from the practical `/film/` production-info desk. **Health** `/health/` is exposure-literacy (hair dye; HSP), not clinical advice. `/extra/` holds adult-education side paths deliberately off the front door. All are inventoried because they exist on main; none authorize fake outcome metrics.

## 8.17 Chapter claim (Draft 1.3 restatement)

**Claim C8′:** The mid-2026 living site does not dilute Owned Pipelines; it widens the *attention and exactness* surface the thesis already needed. Photography still trains light. panatau.com still trains cultural IA. Local/weather/sky/dial/suggest desks train **placeful, shareable, disclaimer-first instruments**. Interpretive parallels to engine ownership are Grade E. URLs, prices, HTTP codes, and pending PRs are Grade A fetches.
...
9. **Attention craft** — photography, cultural systems, and (Draft 1.3) living local/weather/sky/dial/suggest desks.
...
| Attention adjacency | Public media sites | stevenphilley.com (Vol. I + living desks); panatau.com |
...
9. **Attention craft** — nine-image California sequence; panatau lesson architecture; Draft 1.3 living desks (Local, Weather `?zip=`, astronomy cluster, Annual Dial, suggest/sell/suggestpage).
...
## 9.16 Draft 1.3 synthesis addendum — living desks as handshake practice

Draft 1.3 does not move the identity-bearing layers of Ra, Ablockalypse, or Digiflux. It densifies **layer 9** of the stack with public handles:

1. **Place instruments** — Campbell-default weather + share `?zip=`; Pruneyard orchards/arsenic/fill literacy; quake and liquefaction essays with USGS bounds.
...
6. Attention-adjacency reading of photo/cultural sites and living stevenphilley.com desks  
7. Self-directed doctoral form with repeated non-credential framing  

## 10.4 Closing

Computing culture did not begin in app stores, and it will not end in them. Somewhere between a Telnet banner, a CameraUBO field list, a coyote-time constant, a Pacific cliff photograph, and a Campbell ZIP on a shared weather URL, a single practice can still choose to own the handshakes. That choice is not a degree. It is a discipline. This monograph exists to make that discipline examinable.

If the author later deposits a revised edition with first-person chronology and measured appendices, the public-artifact spine of Draft 1.0 should remain intact: still verifiable, still disclaimer-first, still allergic to fake precision. Draft 1.3’s living-site layer is the same spine, widened—not a new credential story.
...
- Day-scale BBS count movement (1304→1322→1323).
...
> Philley, Steven. *Owned Pipelines: Practice-Based Knowledge from Thirty Years of Hands-On Computing.* Practice-based doctoral dissertation (self-directed). Color It Company LLC / Flux Studios, Campbell, CA. Draft 1.3, September 2026. Markdown monograph. Prior drafts 1.2 (2026-09-10), 1.1 and 1.0 (2026-09-09) remain historical files.
...
[^cic-home]: Color It Company. Studio home / Independent Media & Games. https://coloritcompany.com/ now redirects to https://colorit.co/ (verified 2026-09-15). Historical citations in earlier drafts used the coloritcompany.com hostname.  
...
[^cic-bbs]: Color It Company. *Worldwide active BBS directory*. Historical handle https://coloritcompany.com/bbs ; live fetch 2026-09-15 follows to https://colorit.co/bbs/ .  
...
| Photography | California light studies (Vol. I) | https://stevenphilley.com/ | Ch8 |
| Living site changelog | What’s new log | https://stevenphilley.com/whats-new/ | Ch8 |
| Local desk | Campbell traffic, weather, AQI, starters | https://stevenphilley.com/local/ | Ch8 |
| Pruneyard land history | Orchards, Union Ave, arsenic, marsh fill | https://stevenphilley.com/local/pruneyard/ | Ch8 |
| Bay Area earthquakes | Educational quake timeline | https://stevenphilley.com/local/earthquakes/ | Ch8 |
...
| Moon phases | Synodic-month dial | https://stevenphilley.com/science/astronomy/moon/ | Ch8 |
| Local sky | Campbell / Bay Area chart | https://stevenphilley.com/science/astronomy/local-sky/ | Ch8 |
| Nearest stars | Neighborhood distances | https://stevenphilley.com/science/astronomy/nearest-stars/ | Ch8 |
| Exoplanets | Discovery methods + examples | https://stevenphilley.com/science/astronomy/exoplanets/ | Ch8 |
| Galaxy | Milky Way starter | https://stevenphilley.com/science/astronomy/galaxy/ | Ch8 |
| Atmosphere | Clickable air layers | https://stevenphilley.com/science/atmosphere/ | Ch8 |
...
| BBS JSON (colorit.co) | Live directory data (follow-redirect) | https://colorit.co/bbs/bbslist.json | Ch7 |
| Maths | Trig / linear algebra / calculus | https://stevenphilley.com/maths/ | Ch8 |
| Tools | Converters hub | https://stevenphilley.com/tools/ | Ch8 |
| Temperature | °F ↔ °C | https://stevenphilley.com/tools/temperature.html | Ch8 |
| Games hub | Browser games | https://stevenphilley.com/games/ | Ch8 |
| Electronics | Interactive bench | https://stevenphilley.com/electronics/ | Ch8 |
| Film studies | Academic cinema desk | https://stevenphilley.com/filmstudies/ | Ch8 |
| Dissertation reader | This monograph | https://stevenphilley.com/dissertation/ | Front matter |
...
17. ✅ **BBS schema + probe ethics** — Draft 1.2 schema/field frequencies + Snapshots A/B; Draft 1.3 Snapshot C + colorit.co cutover.  
...
*End of Draft 1.3 — Practice-based doctoral dissertation (self-directed). Not an institutional degree award.*
...
Draft 1.0 deliberately withheld numbers the public record did not provide; Draft 1.1–1.2 add only numbers now citable from public headers, storefronts, manuals, and JSON snapshots (e.g., Input.h 180/45; Launch Up 8.5/0.15/0.12/−18/100; BBS 1304/1092 and 1322/1099). Draft 1.3 adds Snapshot C (1323 / statusCounts 1091 green on a 2026-09-12 generate) and living-site URL inventory only—no fabricated desk metrics. Future empirical appendices—if Steven measures and publishes them—might include:
...
Typeset as Markdown for revision velocity. Primary evidence window: September 2026 public web sources listed in References and Appendix A, deepened via `/workspace/dissertation/01-expansion-sources.md` and `/workspace/dissertation/02-draft12-sources.md`, live digiflux/Steam/GitHub/BBS/blog/manual fetches, live stevenphilley.com / colorit.co / suggestpage.com fetches on 2026-09-15, and local shallow clone of `electrosy/sdl2-blocks` under `/workspace/dissertation/vendor/sdl2-blocks` (Draft 1.2 vendor cites retained). Author: Steven Philley. Studio: Color It Company LLC / Flux Studios, Campbell, California. This file is **Draft 1.3** of a practice-based doctoral dissertation (self-directed). It is not a university thesis deposit and not a conferred PhD credential. Draft 1.2 remains in-tree as `Owned_Pipelines_Draft_1.2.md`.

**Draft 1.3 word count:** see Appendix E changelog row (computed with `wc -w`). Target band still ~35k–45k *if and only if* content stayed substantive; fabrication forbidden to hit the number. Prefer stopping short over padding.
